...
The current loop is: plan, dry-run, confirm, execute, inspect, retry, and safely undo supported BlendeX-owned Geometry Nodes changes from CodeX.
...
- batch history 支持检查最近操作，并在可安全回滚时提供 conservative undo last batch。
...
- 验证可安全回滚的 BlendeX batch 确实撤销了场景变更。
...
- 安全和可靠性
  - 更完整的 ownership 规则。
  - 操作批处理验证。
  - undo last batch for supported reversible batches。
  - 用户确认机制。
  - 本地连接鉴权。
